...
def login():
    if request.method == 'POST':
        ...
        login_as_admin = request.form.get('login_as_admin', '0')
        ...
        if user and check_password_hash(user['password'], password):
            ...
            session['business_name'] = user.get('business_name') or user['username']
            flash('Logged in successfully!', 'success')
            # Redirect admin to admin panel if they clicked "Login as Admin"
            if login_as_admin == '1' and user.get('is_admin'):
                return redirect(url_for('admin_panel'))
            elif login_as_admin == '1' and not user.get('is_admin'):
...
def dashboard():
    ...
    business_name = session.get('business_name', session.get('username', 'My Business'))
    conn.close()
    return render_template('dashboard.html', total_items=total_items, total_quantity=total_quantity,
                           total_profit_potential=total_profit_potential, business_name=business_name)
...
def admin_panel():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT id, username, business_name, status, is_admin FROM users ORDER BY id')
...
def admin_create_user():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('INSERT INTO users (username, password, business_name, status, is_admin) VALUES (%s, %s, %s, %s, %s)',
                      (username, hashed_password, business_name or username, 'active', False))
        conn.commit()
        flash(f'Account created for "{username}" ({business_name or username}) successfully!', 'success')
    except Exception:
        conn.rollback()
...
def delete_sale():
    ...
    try:
        ...
        conn.close()
    return redirect(url_for('daily_sales_summary') + '?tab=history')
...
def cash_flow():
    ...
    # --- Money in from SALES ---
    cursor.execute(
        'SELECT id, total_amount, sale_date, items_sold FROM sales WHERE user_id=%s ORDER BY sale_date DESC, id DESC',
        (user_id,)
    )
    sales_rows = cursor.fetchall()

    total_from_sales = 0.0
    sales_entries = []
    for r in sales_rows:
        try:
            items = json.loads(r['items_sold'])
            item_names = ', '.join(i['name'] for i in items) if items else 'Sale'
        except Exception:
            item_names = 'Sale'
    ...
    # --- Money in from RECEIVABLE PAYMENTS ---
    cursor.execute(
        '''SELECT t.amount, t.transaction_date, t.description, rv.customer
           FROM transactions t
           JOIN receivables rv ON t.receivable_id = rv.id
           WHERE rv.user_id=%s AND t.amount > 0
           ORDER BY t.transaction_date DESC''',
        (user_id,)
    )
    rx_rows = cursor.fetchall()

    total_from_receivables = 0.0
    rx_entries = []
    for r in rx_rows:
        total_from_receivables += r['amount']
        rx_entries.append({
            'date': str(r['transaction_date']),
            'source': f"Receivable — {r['customer']}",
            'description': r['description'] or f"Payment from {r['customer']}",
            'amount': r['amount'],
            'type': 'receivable'
        })
    ...
    cursor.execute(
        'SELECT SUM(remaining_amount) FROM receivables WHERE user_id=%s AND remaining_amount > 0.005',
        (user_id,)
    )
    result = cursor.fetchone()
    total_outstanding_receivables = result['sum'] or 0.0

    conn.close()

    # Merge and sort all entries by date descending
    cash_flow_entries = sorted(
        sales_entries + rx_entries,
        key=lambda x: x['date'],
        reverse=True
    )

    total_cash_in = total_from_sales + total_from_receivables
    ...
    return render_template(
        ...
        total_from_receivables=total_from_receivables,
        total_outstanding_receivables=total_outstanding_receivables,
        total_sales_count=len(sales_entries),
        total_receivable_payments=len(rx_entries),
        all_months=months_seen
    )
...
def setup_admin():
    ...
    try:
        ...
        conn.commit()
        return 'Admin setup complete! business_name column added.'
    except Exception as e:
        conn.rollback()
